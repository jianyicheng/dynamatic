...
DFnetlist::DFnetlist(const string &name, const string &name_bb)
    : DFI(new DFnetlist_Impl(name, name_bb)) {}

DFnetlist::DFnetlist(const DFnetlist &other)
    : DFI(new DFnetlist_Impl(*(other.DFI))) {}

DFnetlist::~DFnetlist() { delete DFI; }

bool DFnetlist::check() { return DFI->check(); }

const string &DFnetlist::getName() const { return DFI->getName(); }
...
int DFnetlist::getLatency(blockID id) const { return DFI->getLatency(id); }

void DFnetlist::setLatency(blockID id, int lat) { DFI->setLatency(id, lat); }
...
double DFnetlist::getTrueFrac(blockID id) const { return DFI->getTrueFrac(id); }
...
void DFnetlist::removeBlock(blockID block) { return DFI->removeBlock(block); }

portID DFnetlist::createPort(blockID block, bool isInput, const string &name,
                             int width, PortType type) {
  return DFI->createPort(block, isInput, name, width, type);
}

void DFnetlist::removePort(portID port) { DFI->removePort(port); }
...
portID DFnetlist::getTruePort(blockID id) const { return DFI->getTruePort(id); }
...
portID DFnetlist::getSrcPort(channelID id) const { return DFI->getSrcPort(id); }

portID DFnetlist::getDstPort(channelID id) const { return DFI->getDstPort(id); }

void DFnetlist::removeChannel(channelID id) { return DFI->removeChannel(id); }
...
bool DFnetlist::hasBuffer(channelID id) const { return DFI->hasBuffer(id); }

blockID DFnetlist::insertBuffer(channelID c, int slots, bool transparent) {
  return DFI->insertBuffer(c, slots, transparent);
}

void DFnetlist::removeBuffer(blockID buf) { DFI->removeBuffer(buf); }

void DFnetlist::setError(const string &err) { DFI->setError(err); }

const string &DFnetlist::getError() const { return DFI->getError(); }

void DFnetlist::clearError() { DFI->clearError(); }

bool DFnetlist::hasError() const { return DFI->hasError(); }

int DFnetlist::numBlocks() const { return DFI->numBlocks(); }

int DFnetlist::numChannels() const { return DFI->numChannels(); }

int DFnetlist::numPorts() const { return DFI->numPorts(); }

bool DFnetlist::validPort(portID p) const { return DFI->validPort(p); }

bool DFnetlist::validBlock(blockID id) const { return DFI->validBlock(id); }
...
bool DFnetlist::writeDot(std::ostream &s) { return DFI->writeDot(s); }

bool DFnetlist::writeDotMG(const std::string &filename) {
  return DFI->writeDotMG(filename);
}

bool DFnetlist::writeDotMG(std::ostream &s) { return DFI->writeDotMG(s); }

bool DFnetlist::writeDotBB(const std::string &filename) {
  return DFI->writeDotBB(filename);
}

bool DFnetlist::writeDotBB(std::ostream &s) { return DFI->writeDotBB(s); }
...
bool DFnetlist::addElasticBuffers(double Period, double BufferDelay,
                                  bool maxThroughput, double coverage) {
  return DFI->addElasticBuffers(Period, BufferDelay, maxThroughput, coverage);
}

bool DFnetlist::addElasticBuffersBB_sc(double Period, double BufferDelay,
                                       bool maxThroughput, double coverage,
                                       int timeout, bool first_MG) {
  return DFI->addElasticBuffersBB_sc(Period, BufferDelay, maxThroughput,
                                     coverage, timeout, first_MG);
}

bool DFnetlist::addElasticBuffersBB(double Period, double BufferDelay,
                                    bool maxThroughput, double coverage,
                                    int timeout, bool first_MG) {
  return DFI->addElasticBuffersBB(Period, BufferDelay, maxThroughput, coverage,
                                  timeout, first_MG);
}
...
void DFnetlist::hideElasticBuffers() { DFI->hideElasticBuffers(); }

void DFnetlist::cleanElasticBuffers() { DFI->cleanElasticBuffers(); }

void DFnetlist::eraseNonSCC() { DFI->eraseNonSCC(); }

bool DFnetlist::calculateBasicBlocks() { return DFI->calculateBasicBlocks(); }

bool DFnetlist::optimize() { return DFI->optimize(); }

bool DFnetlist::removeControl() { return DFI->removeControl(); }
...
void DFnetlist::printBlockSCCs() { DFI->printBlockSCCs(); }
...
bool DFlib::add(const std::string &filename) { return DFLI->add(filename); }

DFlib::~DFlib() { delete DFLI; }

bool DFlib::writeDot(const std::string &filename) {
  return DFLI->writeDot(filename);
}

void DFlib::setError(const string &err) { DFLI->setError(err); }

const string &DFlib::getError() const { return DFLI->getError(); }

void DFlib::clearError() { DFLI->clearError(); }

bool DFlib::hasError() const { return DFLI->hasError(); }

int DFlib::numFuncs() const { return DFLI->numFuncs(); }
...
DFnetlist &DFlib::operator[](funcID id) { return (*DFLI)[id]; }

const DFnetlist &DFlib::operator[](funcID id) const { return (*DFLI)[id]; }
