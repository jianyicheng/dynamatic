...
int main_buffers(const vecParams &params) {
  if (params.size() != 5) {
    cerr << "Usage: " + exec + ' ' + command +
                " period buffer_delay solver infile outfile"
         << endl;
    return 1;
  }
  ...
  cout << "Adding elastic buffers with period=" << period
       << " and buffer_delay=" << delay << endl;
  DF.setMilpSolver(params[2]);

  bool stat = DF.addElasticBuffers(period, delay, true);

  // Lana 05/07/19 Instantiate if previous step successful, otherwise just print
  // and exit
  if (stat) {
...
int main_help() {
  ...
  cerr << "  solveCSC:      solve state encoding in an asynchronous circuit."
       << endl;
  cerr << "  hideSignals:   hide signals in an asynchronous specification."
       << endl;
  cerr << "  syncprod:      calculate the synchronous product of two LTSs."
       << endl;
  cerr << "  bisimilar:     check whether two LTSs are weakly bisimilar."
       << endl;
...
void show_help_shab() {
  cout << "-filename: <filename>_graph.dot and <filename>_bbgraph.dot should "
          "contain DFC and CFG respectively"
       << endl;
  cout << "\toptimized output will be writen to <filename>_graph_buf.dot and "
          "<filename>_bbgraph_buf.dot"
       << endl;
  ...
  cout << "-first: whether the milp should only consider the throughput for "
          "the first MG or not"
       << endl;
  cout << "\tdefault value is false" << endl;
}
int main_shab(const vecParams &params) {
  ...
  cout << "Adding elastic buffers with period=" << input.period
       << " and buffer_delay=" << input.delay << endl;
  cout << endl;
  DF.setMilpSolver(input.solver);

  bool stat;
  if (input.set) {
    stat = DF.addElasticBuffersBB_sc(input.period, input.delay, true, 1,
                                     input.timeout, input.first);
  } else {
    stat = DF.addElasticBuffersBB(input.period, input.delay, true, 1,
                                  input.timeout, input.first);
  }
  if (stat) {
    DF.instantiateElasticBuffers();
...
// Jianyi 170322: Added as a parser/formatter
int main_format(const vecParams &params) {
  if (params.size() == 1 && params[0] == "-help") {
    show_help_shab();
    return 1;
  }
  ...
  cout << "Reformating dot graph..." << endl;
  cout << endl;
  DF.writeDot(input.graph_name + "_graph_buf.dot");
  DF.writeDotBB(input.graph_name + "_bbgraph_buf.dot");
  return 0;
...
int main(int argc, char *argv[]) {
  ...
  if (command == "buffers_old")
    return main_buffers(params);
  if (command == "buffers")
    return main_shab(params);
  if (command == "test")
    return main_test(params);
  if (command == "format")
    return main_format(params);
